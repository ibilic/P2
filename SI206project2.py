...
def grab_headlines():
    ...
    lst = list()
    most_viewed = soup.find_all(class_="view view-most-read view-id-most_read view-display-id-panel_pane_1 view-dom-id-99658157999dd0ac5aa62c2b284dd266")
    first_thing = most_viewed[0]
    link = first_thing.find_all('a')
    for content in link:
        title = content.text
        lst.append(title)
    return lst
...
def get_umsi_data():
    ...
    for link in lst:
        if link == ' ':
            ...
            url = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page=" + '&page=' + str(link)
        ...
        title = soup.find_all(class_ = "field field-name-field-person-titles field-type-text field-label-hidden")

        x = [line.text for line in name]
        y = [line.text for line in title]
        zipped = list(zip(x,y))
        p.append(zipped)
    combined = dict(itertools.chain.from_iterable(p))
